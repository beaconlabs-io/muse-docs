...
引用文献セクションには、このエビデンスエントリーで参照された文献が一覧表示されます。URLを含む引用はクリック可能なボタンとして表示され、ソースに直接アクセスできます。その他の引用はテキストとして表示されます。元の研究論文を読んだり、発見事項を検証したりしたい場合は、ここで引用情報を確認できます。
...
エビデンスページの下部には変更履歴セクションがあります。ここでは、このエビデンスエントリーのブロックチェーン検証記録が表示されます。

各記録には以下が含まれます。

- **EAS UID** — Ethereum Attestation Service（EAS）ブロックチェーン上の固有識別子で、このエビデンスエントリーを永続的かつ公開的に記録します（クリックするとEASで確認できます）
- **タイムスタンプ** — アテステーションが作成された正確な日時
- **Latestバッジ** — 最新のアテステーションには「Latest」バッジが表示されます

:::info アテステーションとは？
アテステーションとは、公開ブロックチェーンに記録された暗号署名で、「このエビデンスエントリーはこの時点でこの正確な形式で存在していた」ということを証明するものです。これによりエビデンスは改ざん耐性を持ち、独立して検証可能になります — 誰でもブロックチェーン上のEAS UIDを確認して、記録が本物であることを確かめることができます。
:::

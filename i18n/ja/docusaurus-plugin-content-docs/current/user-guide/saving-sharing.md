...
sidebar_position: 7
title: 保存と共有
description: MUSEでロジックモデルや測定レシピを保存・エクスポート・共有する方法
---

# 保存と共有

ロジックモデルが完成したら、MUSEにはそれを保存・共有するための複数の方法があります。IPFSへの永続保存で共有リンクを取得する方法、プレゼンテーション用の画像エクスポート、JSONとしての生データのダウンロード、[測定レシピ](./recipe) をHTMLでプログラム運営者に手渡す方法、またはブラウザへの自動ローカル保存を活用できます。
...
![アップロード成功後のIPFS保存ダイアログ。IPFSハッシュ、共有用URL、キャンバスのサムネイル、コピーボタンとX共有ボタンが表示されている。](/img/screenshots/ja/ipfs-save-dialog.png)
...
エクスポートされたPNGの隅には、小さな **「MUSE」** のクレジットが埋め込まれます。これは意図的な仕様で、外部共有や報告書掲載時にエクスポート元がMUSEであることを追跡できるようにするためです。

:::tip 画像エクスポートのベストプラクティス
エクスポートする前に、ズームアウトしてミニマップを確認し、すべてのノードが希望通りに配置されているか確認してください。エクスポートはキャンバスの現在の配置を反映します。編集で配置が乱れているときは、先に **More → Auto Layout** で整えてからエクスポートするとよいでしょう。
:::
...
キャンバスに対して [測定レシピ](./recipe) を生成済みであれば、自己完結型のHTMLファイルとしてプログラム運営者に手渡すことができます。

### レシピのダウンロード方法

1. レシピが生成済みであることを確認する（Recipeタブに空の状態ではなく、レシピの内容が表示されている状態）
2. ツールバーの **「More」** ボタンをクリックする
3. **Recipe** セクションの **「Download HTML」** をクリックする
4. 単一の `.html` ファイルがブラウザのダウンロードフォルダーに保存される

このファイルは完全に自己完結しており — 表示に外部アセットは不要 — メール添付・メッセージング共有・アーカイブにそのまま利用できます。

:::info Download HTMLが無効な場合
レシピを一度も生成していない、または前回の生成が失敗した場合、**Download HTML** メニュー項目は無効になっています。先にRecipeタブに切り替えて生成してください。
:::
...
| プログラム運営者に測定計画を手渡す | レシピを生成 → HTMLダウンロード |
